...
const calculateMatchScore = (product, requestData) => {
  ...
  if (
    normalizeText(product.mainProblem) &&
    normalizeText(mainProblem) &&
    normalizeText(product.mainProblem) === normalizeText(mainProblem)
  ) {
    score += 40;
    reasons.push("Sesuai dengan masalah utama usaha");
  } else if (
    hasTextOverlap(product.mainProblem, mainProblem) ||
    hasTextOverlap(product.problemDetail, problemDetail) ||
    hasTextOverlap(product.description, problemDetail)
  ) {
    score += 25;
    reasons.push("Masalah produk masih berhubungan dengan kebutuhan UMKM");
  }
  ...
  const requestDuration = Number(trainingDuration);

  if (
    !Number.isNaN(productDuration) &&
    !Number.isNaN(requestDuration) &&
    productDuration <= requestDuration
  ) {
    score += 10;
  ...
  if (
    ...
    hasTextOverlap(product.targetBusiness, keyword)
  ) {
    score += 5;
